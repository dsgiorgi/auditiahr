# main.py — AuditIA (procesamiento SIEMPRE en Azure)
...
# === Config obligatoria: endpoint de Azure Function que anonimiza ===
# Ej.: ANON_API_URL="https://auditiahr-func.azurewebsites.net/api/anon-upload"
ANON_API_URL = os.getenv("ANON_API_URL", "").strip()
ANON_API_KEY = os.getenv("ANON_API_KEY", "").strip()  # si tu Function usa key

if not ANON_API_URL:
    st.stop()  # corta la app con un mensaje claro
    raise SystemExit("Falta configurar ANON_API_URL en variables de entorno o st.secrets")

def upload_and_anon_in_azure(file_like, filename: str) -> bytes:
    """
    Envía el archivo (CSV/XLSX ya consolidado si aplica) a la Azure Function
    que lo anonimiza (y opcionalmente persiste en Blob). Debe devolver CSV anonimizado (bytes).
    """
    headers = {}
    if ANON_API_KEY:
    ...
    files = {
        "file": (
            filename,
            file_like if hasattr(file_like, "read") else pyio.BytesIO(file_like),
            "text/csv" if filename.lower().endswith(".csv") else "application/octet-stream",
        )
    }
    resp = requests.post(ANON_API_URL, headers=headers, files=files, timeout=90)
    ...
    # Soporte de respuesta JSON { csv_base64, blob_url }
    try:
        j = resp.json()
        if "csv_base64" in j:
            import base64
        ...
        if "blob_url" in j:
            return requests.get(j["blob_url"], timeout=60).content
    except Exception:
        pass
...
if mode == "Un archivo con etapas":
    ...
    if f:
        try:
            with st.spinner("🔒 Enviando a Azure para anonimizar..."):
                # Nota: enviamos el binario tal como llega al backend de Azure
                csv_bytes = upload_and_anon_in_azure(f, f.name if hasattr(f, "name") else "dataset.csv")
            df = pd.read_csv(pyio.BytesIO(csv_bytes))
            mapping, _ = io.smart_map_columns(df)
            st.success("✅ Procesado en Azure y cargado (dataset ya anonimizado).")
            if not hr_mode:
                st.dataframe(df.head(8), use_container_width=True)
        except Exception as e:
            st.error(f"Error al procesar en Azure: {e}")

else:  # múltiples archivos
    files = st.file_uploader(
        "Subí uno o más archivos (CSV/XLSX)",
        type=["csv", "xlsx"],
    ...
    if files:
        ...
        # 1) combinamos en memoria (solo para estructurar etapas)
        try:
            df_local = io.merge_files_with_stages(list(zip(files, labels)))
            if df_local is None or df_local.empty:
                st.error("No se pudieron combinar los archivos.")
            else:
                # 2) exportamos a CSV en memoria y lo enviamos a Azure para anonimizar
                with st.spinner("🔒 Consolidando y enviando a Azure para anonimizar..."):
                    csv_in = df_local.to_csv(index=False).encode("utf-8-sig")
                    csv_bytes = upload_and_anon_in_azure(pyio.BytesIO(csv_in), "consolidado.csv")
                df = pd.read_csv(pyio.BytesIO(csv_bytes))
                mapping, _ = io.smart_map_columns(df)
                st.success("✅ Consolidado procesado en Azure y cargado (dataset ya anonimizado).")
                if not hr_mode:
                    st.dataframe(df.head(8), use_container_width=True)
        except Exception as e:
            st.error(f"No se pudieron preparar/enviar los archivos: {e}")
...
if df is not None and not df.empty:
    ...
    # (NO hay checkbox de anonimización: el dataset que ves YA viene anonimizado desde Azure)

    if "edad" in df.columns:
        with st.expander("Opcional: agrupar EDAD en rangos (recomendado)", expanded=True):
            modo_age = st.radio(
                "Cómo agrupar edad:", ["Automático (de 10 años)", "Personalizado (elegir cortes)", "Sin agrupar"],
    ...
    if ready:
        ...
        # Sugerir columnas sensibles a comparar
        candidatos = []
        ...
        if "edad_rango" in df.columns:
            if "edad" in candidatos:
                candidatos.remove("edad")
            if "edad_rango" not in candidatos:
                candidatos.insert(0, "edad_rango")
        ...
        if not candidatos:
            ...
            candidatos_sel = st.multiselect(
                ...
                options=candidatos,
                default=candidatos,
                help="Podés destildar los que no quieras analizar.",
            )
            ...
            for colname, tab in zip(cols_a_usar, tabs):
                with tab:
                    ...
                    if rates:
                        ...
                        fig = px.bar(
                            dfr, x="Grupo", y="Tasa de avance", text="Tasa de avance", range_y=[0, 1], color="Grupo"
                        )
                        ...
                        for sname, stab in zip(etapas, stage_tabs):
                            with stab:
                                ...
                                if rates_s:
                                    ...
                                    fig2 = px.bar(
                                        dt, x="Grupo", y="Tasa de avance", text="Tasa de avance", range_y=[0, 1], color="Grupo"
                                    )
        ...
        auto_features, roles = heuristics.suggest_features(
            df, outcome_col=outcome_col, y_true_col=y_true_col if y_true_col != "(ninguna)" else None
        )
            ...
            if not auto_features:
                        ...
                        try:
                            if s.dtype.kind in "biufc" and s.nunique(dropna=True) > 10:
                                ...
                                if len(low) >= 15 and len(high) >= 15:
                                    ...
                                    bullets.append(
                                        f"• Valores altos de **{feat}** {direction} la probabilidad de avanzar en ~**{abs(gap):.1f} p.p.** (Q4={r_high:.2f} vs Q1={r_low:.2f})."
                                    )
                            else:
                                ...
                                if len(rates) >= 2:
                                    ...
                                    gap = (r_b - r_w) * 100
                                    bullets.append(
                                        f"• En **{feat}**, **{best}** avanza ~**{r_b:.2f}** vs **{worst}** ~**{r_w:.2f}** (gap ~**{gap:.1f} p.p.**)."
                                    )
                        except Exception:
                            continue
                ...
                if not hr_mode:
                    try:
                        ...
                        if imp:
                            imp_df = pd.DataFrame(
                                sorted(imp.items(), key=lambda x: x[1], reverse=True)[:15],
                                columns=["Variable", "Importancia relativa"],
                            )
                            figi = px.bar(
                                imp_df.sort_values("Importancia relativa"),
...
"""
        ...
        csv_bytes_out = df.to_csv(index=False).encode("utf-8-sig")
        st.download_button(
